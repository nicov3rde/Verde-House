...
export const actions: Actions = {
	register: async (event) => {
		...
		if (!displayName || !handle || !email || !password) {
			return fail(400, { error: 'All fields are required.', displayName, handle, email });
		}
		...
		if (password.length < 8) {
			return fail(400, { error: 'Password must be at least 8 characters.', displayName, handle, email });
		}

		// Check existing
		const existing = await db.select().from(users).where(eq(users.email, email)).limit(1);
		if (existing.length) {
			return fail(400, { error: 'An account with that email already exists.', displayName, handle, email });
		}

		const existingHandle = await db.select().from(users).where(eq(users.handle, handle)).limit(1);
		if (existingHandle.length) {
			return fail(400, { error: 'That handle is already taken.', displayName, handle, email });
		}
